...
public:
    ...
    double MassAccuracy = LibraryCommon::GlobalMassAccuracy, MassAccuracyMs1 = LibraryCommon::GlobalMassAccuracyMs1;
    ...
    template <class F> void read_meta(F &in, int _lib_size = 0) {
        ...
        ReadWriteCommonFunctons::read_vector(in, MassCorrection);
        ReadWriteCommonFunctons::read_vector(in, MassCorrectionMs1);
        ReadWriteCommonFunctons::read_vector(in, MassCalSplit);
        ReadWriteCommonFunctons::read_vector(in, MassCalSplitMs1);
        ReadWriteCommonFunctons::read_vector(in, MassCalCenter);
        ReadWriteCommonFunctons::read_vector(in, MassCalCenterMs1);
    }
    ...
    template <class F> void read(F &in, int _lib_size = 0) {
        read_meta(in, _lib_size);
        ReadWriteCommonFunctons::read_vector(in, proteins);
        ReadWriteCommonFunctons::read_vector(in, detectable_precursors);
...
public:
    ...
    public:
        ...
        float qvalue = 0.0;
        float pg_qvalue = 0.0;
        float best_fr_mz = 0.0;

        int MinFrAAs = 3;
        double MinFrMz = 200.0;
        double MaxFrMz = 1800.0;
        double MinPrMz = 300.0;
        double MaxPrMz = 1800.0;
        ...
        inline void generate_decoy() {
            ...
            if (recognise || LibraryCommon::ReverseDecoys) {
                ...
                pattern = LibraryCommon::recognise_fragments(
                        name,
                        target.fragments,
                        gen_acc,
                        target.charge,
                ...
                if (gen_acc > LibraryCommon::GeneratorAccuracy + std::numeric_limits<double>::min() && LibraryCommon::ForceFragRec) {
            ...
            if (!LibraryCommon::ReverseDecoys) {
                ...
                auto mod_state = LibraryCommon::get_mod_state(name, aas.length());

                if (!mod_state[1]) pos = 1;
                else if (!mod_state[std::min(2, m + 1)]) pos = std::min(2, m + 1);
                else if (!mod_state[0]) pos = 0;
                else pos = 1;
                N_shift = LibraryCommon::AA[LibraryCommon::MutateAAto[LibraryCommon::AA_index[aas[pos]]]] - LibraryCommon::AA[aas[pos]];

                if (!mod_state[m]) pos = m;
                else if (!mod_state[std::max(0, m - 1)]) pos = std::max(0, m - 1);
                else if (!mod_state[m + 1]) pos = m + 1;
                else pos = m;
                C_shift = LibraryCommon::AA[LibraryCommon::MutateAAto[LibraryCommon::AA_index[aas[pos]]]] - LibraryCommon::AA[aas[pos]];

                for (i = 0; i < decoy.fragments.size(); i++) {
                    double c = static_cast<double>(std::max(decoy.fragments[i].charge, static_cast<char>(1)));
                    if (decoy.fragments[i].type & LibraryCommon::fTypeY) decoy.fragments[i].mz += C_shift / c;
                ...
                for (i = 0; i < seq.size(); i++) if (std::abs(inv_seq[i] - seq[i]) > 1200.0 * LibraryCommon::GeneratorAccuracy) break;
                if (i == seq.size()) inv_seq[1] += 12.0, inv_seq[seq.size() - 2] -= 12.0;

                auto dfr = LibraryCommon::generate_fragments(inv_seq, pattern, target.charge);
        ...
        inline void annotate_charges() {
            ...
            auto pattern = LibraryCommon::recognise_fragments(
                    seq,
                    target.fragments,
                    gen_acc,
                    target.charge,
        ...
        std::vector<Ion> fragment() {
            ...
            for (int charge = 1; charge <= 2; charge++) {
                for (int loss = loss_none; loss <= loss_NH3; loss++) {
                    auto frs = LibraryCommon::generate_fragments(seq, charge, loss, &cnt, MinFrAAs);
        ...
        void generate() {
            ...
            auto seq = LibraryCommon::get_sequence(name);
            auto aas = LibraryCommon::get_aas(name);
            auto gen = LibraryCommon::generate_all_fragments(seq, 1, AddLosses ? loss_NH3 : loss_none, &cnt);
            std::vector<double> scores, H2O_scores, NH3_scores;
            LibraryCommon::y_scores(scores, target.charge, aas), LibraryCommon::to_exp(scores);

            if (AddLosses) {

                LibraryCommon::y_loss_scores(
                        H2O_scores,
                        aas,
                        false
                        );
                ...
                LibraryCommon::to_exp(H2O_scores);
                LibraryCommon::to_exp(NH3_scores);
            }

            for (i = cnt = 0; i < gen.size(); i++)
                if (gen[i].mz >= MinFrMz && gen[i].mz <= MaxFrMz) if ((gen[i].type & LibraryCommon::fTypeY) && seq.size() - gen[i].index >= MinFrAAs) cnt++;
            target.fragments.resize(cnt);

            for (i = cnt = 0; i < gen.size(); i++)
                if (gen[i].mz >= MinFrMz && gen[i].mz <= MaxFrMz) if ((gen[i].type & LibraryCommon::fTypeY) && seq.size() - gen[i].index >= MinFrAAs) {
    ...
    template<class F> void read(F &in) {
        ...
        ReadWriteCommonFunctons::read_string(in, name);
        ReadWriteCommonFunctons::read_string(in, fasta_names);
        ReadWriteCommonFunctons::read_array(in, proteins);
        ReadWriteCommonFunctons::read_array(in, protein_ids);
        ReadWriteCommonFunctons::read_strings(in, precursors);
        ReadWriteCommonFunctons::read_strings(in, names);
        ReadWriteCommonFunctons::read_strings(in, genes);
        in.read((char*)&iRT_min, sizeof(double));
        in.read((char*)&iRT_max, sizeof(double));
        ReadWriteCommonFunctons::read_array(in, entries);
        ...
        if (version <= -1 && in.peek() != std::char_traits<char>::eof()) {
            ReadWriteCommonFunctons::read_vector(in, elution_groups);
        }
    }
...
//                    if (in.fail() || temp_folder.size()) in = std::ifstream(location_to_file_name(files[i]) + std::string(".quant"), std::ifstream::binary);
    ...
    void assemble_elution_groups() {
        ...
        for (auto &e : entries) {
            auto name = LibraryCommon::to_eg(e.name);
...
//    }
    ...
    bool load(const char * file_name) {
        ...
        int PGLevel = 2;
        int PGLevelSet = PGLevel; // 0 - ids, 1 - names, 2 - genes
        int colInd[LIB::libCols];

        name = std::string(file_name);

        if (LibraryCommon::get_extension(name) == std::string(".speclib")) {
...
//        else if (LibraryCommon::get_extension(name) == std::string(".sptxt") || LibraryCommon::get_extension(name) == std::string(".msp")) {
//            load_sptxt(file_name, LibraryCommon::SptxtMassAccuracy);
...
//        std::vector<int> max_protein_score(proteins.size(), (int)-std::numeric_limits<double>::max());
//
