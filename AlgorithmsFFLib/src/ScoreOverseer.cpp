...
namespace {
    ...
    Err calculateMS1Corr(
        ...
        const auto ms1MeanFoundLogic = [](float sum, const XICPoint &xp){return sum + xp.mz;};
        *ms1MeanMzFound
            = std::accumulate(xicPoints.begin(), xicPoints.end(), 0.0f, ms1MeanFoundLogic) / static_cast<float>(xicPoints.size());
