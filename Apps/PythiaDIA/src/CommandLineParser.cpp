...
namespace {
    const QString ARG_FRAGLIB_PATH = QStringLiteral("fraglibdf-path");
...
CommandLineParser::CommandLineParser() {
    addHelpOption();
    addPositionalArgument(ARG_FRAGLIB_PATH, QObject::tr("*.fragLibDF file"));
...
bool CommandLineParser::validateArguments(const QStringList &args) {
    ...
    const bool fragLibFilePathIsValid = CommandLineParserUtils::checkFileNameExtension(
            ...
            S_GLOBAL_SETTINGS.DOT_FRAGLIB_DF.mid(1, 9)
            );
    if (!fragLibFilePathIsValid) {
        qCritical() << QStringLiteral("First command line argument *.fragLibDF, argument invalid");
        argumentsLocal.append("-h");
        process(argumentsLocal);
        return false;
    }
    ...
    const bool dataFilesPathIsValid = CommandLineParserUtils::checkFileNameExtension(m_cliParams.msDataFile, "prqDF")
