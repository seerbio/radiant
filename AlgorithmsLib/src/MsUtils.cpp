...
Err MsUtils::calcXCorr(
    ...
    e = ErrorUtils::isNotEmpty(foundMassVals); ree;
    e = ErrorUtils::isNotEmpty(theoMassVals); ree;
    e = ErrorUtils::isNotEmpty(foundIntensityVals); ree;
    e = ErrorUtils::isNotEmpty(theoIntensityVals); ree;

    e = ErrorUtils::isTrue(foundMassVals.size() == foundIntensityVals.size()); ree;
    e = ErrorUtils::isTrue(theoMassVals.size() == theoIntensityVals.size()); ree;

    QVector<double> theoIntesityValsSqrt;
    std::transform(
            theoIntensityVals.begin(),
            theoIntensityVals.end(),
            std::back_inserter(theoIntesityValsSqrt),
            [](double d){return std::sqrt(d);}
    );

    QVector<QPointF> foundPoints;
    e = ParallelUtils::zip(
            foundMassVals,
            foundIntensityVals,
            &foundPoints
    ...
    e = ParallelUtils::zip(
            theoMassVals,
            theoIntesityValsSqrt,
            &theoPoints
            ); ree;

    const ExtractPoints extractedPoints = extractPointsFromPoints(
            foundPoints,
            theoPoints,
            ppmTol
            );

    *xCorr = std::accumulate(
            extractedPoints.intensityFoundVsSearched.begin(),
            extractedPoints.intensityFoundVsSearched.end(),
            0.0,
            [](double sum, const QPointF &pnt){return sum + (pnt.x() * pnt.y());}
            );
