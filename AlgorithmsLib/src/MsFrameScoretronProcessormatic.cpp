...
namespace {
    ...
    Err buildPeptideSeqVsIsDecoyLookup(
            const QVector<FrameExtractsReaderRow> &frameExtractsReaderRows,
            QMap<PeptideStringWithMods, bool> *pepStrWModsVsIsDecoy
            ) {
        ...
        for (const FrameExtractsReaderRow &r : frameExtractsReaderRows) {
            pepStrWModsVsIsDecoy->insert(r.peptideStringWithMods, r.isDecoy);
        }
...
Err MsFrameScoretronProcessormatic::deconvolveScanCandidate(const QVector<FrameExtractsReaderRow> &frameExtractsReaderRows) const {
    ...
    QMap<PeptideStringWithMods, bool> pepStrWModsVsIsDecoy;
    e = buildPeptideSeqVsIsDecoyLookup(
            frameExtractsReaderRows,
            &pepStrWModsVsIsDecoy
            ); ree
...
#define REMOVE_NEG_DISC_SCORES
    ...
    while (negativesArePresent) {
        ...
        const QList<TandemDeconvolverResult> &results = pepSeqVsTandemDeconvolverResult.values();
        negativesArePresent = std::any_of(
                results.begin(),
                results.end(),
                [](const TandemDeconvolverResult &r){return r.discScore < 0.0;}
                );
    }
...
#define WRITE_POINTS
    ...
    for (auto it = pepSeqVsTandemDeconvolverResult.begin(); it != pepSeqVsTandemDeconvolverResult.end(); it++) {

        if (it.value().discScore < 0.05) {
            continue;
        }

        qDebug() << it.key() << it.value().discScore << pepStrWModsVsIsDecoy.value(it.key()) << it.value().pVal;
    }
    ...
    qDebug() << "is decoy" << pepStrWModsVsIsDecoy.value(pep) << res.discScore
             << res.pVal << res.tTestVal << pepStrWModsVsIsDecoy.value(pep);
    qDebug() << "SCORES FRAME" << res.frameFStat << res.pValFrameFtest << res.frameError;
