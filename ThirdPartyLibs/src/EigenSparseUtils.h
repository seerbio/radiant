...
public:
    ...
    */
    ...
    static T max(const Eigen::SparseMatrix<T> &mat){
        ...
        return static_cast<T>(castVec.maxCoeff());
    }
    ...
    */
    ...
    static Eigen::SparseMatrix<int> markMatrixApexes(
        ...
        std::set<int> visitedColumns;
        for (int i = 0; i < mat.outerSize(); ++i) {
            for (typename Eigen::SparseMatrix<T>::InnerIterator it(mat, i); it; ++it) {

                const int column = it.col();

                if(visitedColumns.count(column)){
                    continue;
                }
    ...
    template<typename T>
    static std::vector<SparseMatrixPoint> findMatrixApexes(const Eigen::SparseMatrix<T> &mat) {

        const Eigen::SparseMatrix<int> matMarkedApexes = markMatrixApexes<T>(mat);

        std::vector<SparseMatrixPoint> resultVec;
    ...
    */
    template <typename T>
    static std::map<int, T> apexes(const Eigen::SparseVector<T> &vec,
                               int precision = 1e4){
        std::map<int, T> apexIndicies;
    ...
    template <typename T>
    static std::map<int, T> apexes(const Eigen::SparseVector<T, Eigen::RowMajor> &vec,
                               int precision = 1e4){
    ...
    static void sortApexesHiLoValue(std::vector<SparseMatrixPoint> *apxs);


    // helper method apexes(const Eigen::SparseMatrix<T> &mat)
    template <typename T>
    static std::map<int, std::map<int, T> >  matrixApexesByColumn(const Eigen::SparseMatrix<T> &mat){
        std::map<int, std::map<int, T> > columnApexes;
        std::set<int> visitedColumns;
        for (int i = 0; i < mat.outerSize(); ++i) {
            for (typename Eigen::SparseMatrix<T>::InnerIterator it(mat, i); it; ++it) {
                ...
                const std::map<int, T> colApexes = apexes(colVec);
    ...
    template <typename T>
    static std::vector<SparseMatrixPoint> apexes(
            const Eigen::SparseMatrix<T> &mat,
            int precision = 1e4
                    ){

        const int matrixEdgeBuffer = 2;
        const std::map<int, std::map<int, T>> columnApexes = matrixApexesByColumn(mat);

        std::vector<SparseMatrixPoint> returnApexes;
        for (auto it = columnApexes.begin(); it != columnApexes.end(); it++){
            ...
            const  std::map<int, double> colApexes = it.value();
            for(auto it2 = colApexes.begin(); it2!= colApexes.end(); it2++){
                ...
                const int colApex = static_cast<int>(std::round(it2->second * precision));
                ...
                if(colApex > left
                    ...
                    a.row = row;
                    a.col = column;
                    a.value = static_cast<double>(it2->second);
    ...
    static Eigen::SparseMatrix<T, Eigen::RowMajor> buildCombFilter(
            const std::vector<T> &teethValues,
            T tolerance,
            T maxValue,
            T2 precision
            ) {
