...
namespace timsdata {
    ...
    {
    public:
        using FrameIteratorRange = std::pair<const uint32_t*, const uint32_t*>; // just for this example, could be boost::iterator_range, range-v3 or std::range
    ...
    private:
        ...
        const std::shared_ptr<uint32_t[]> pData; //< data layout as described in tims_read_scans()
    ...
    /// \returns std::string containing the last timsdata.dll error string.
    ...
    public:
        ...
        /// \param[in] use_recalibration if most recent recalibration state shall be used (if available)
        /// \param[in] pressure_compensation the pressure compensation strategy (if available in the data)
        ///
        /// \throws std::exception in case of an error
        explicit TimsData (
            const std::string& analysis_directory_name,
