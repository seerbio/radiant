...
-- Additional sweeping meta-information.
CREATE TABLE CollisionEnergySweepingInfo (
-- The frame to which this information applies.
    Frame INTEGER NOT NULL,

-- An identifier for the ramp value for the current frame.
-- Id's are not neccessarily sorted by energy values
...
-- Percentage of the energy
    CollisionEnergyPercent REAL NOT NULL,

    PRIMARY KEY(Frame, CollisionId),
...
INSERT INTO GlobalMetadata VALUES ('SchemaVersionMinor', '7');
...
CREATE TABLE Frames (
    ...
    -- Measured Pressure of the device. Required to perform a pressure compensated tims calibration
    Pressure REAL,
...
CREATE TABLE PrmTargets (
    ...
    -- A free-text description for display purposes (might contain, e.g., peptide
    -- sequence, sum formula, substance name, SMILES string, etc.), can be an empty string
    Description TEXT NOT NULL
);
...
-- This table describes measurement modes for PRM PASEF frames. For every PRM PASEF frame,
-- a single ID is assignable by external software to identify different frame measurement modes
-- e.g. differing AccumulationTime.
--
-- NOTE: every frame for which there is an entry in this table, will have Frames.MsMsType
-- = 10
CREATE TABLE PrmFrameMeasurementMode (
    ...
    -- Optionally, an ID assignable by external software to identify different frame measurement modes
    -- e.g. differing AccumulationTime.
