...
public:
    ...
    static inline void setMatrix(Matrix* mat, size_t row, size_t col, float val){
    ...
    static void shuffleTogether(DataSet* A, DataSet* B, std::minstd_rand &gen){
    ...
    static inline float getMatrix(Matrix* mat, size_t row, size_t col){
    ...
    static void matrix_mul_add(Matrix * to, float a, Matrix * x, float b, bool square = false) {
    ...
    static void matrix_replace(Matrix * to, Matrix * x, float b) {
        assert(to->rows == x->rows && to->cols == x->cols);
        int i, j;
        for (i = 0; i < to->rows; i++) {
            for (j = 0; j < to->cols; j++)
    ...
    static void adam_update(Matrix * to, Matrix * m, Matrix * v, float a) {
