...
Layer* createLayer(LAYER_TYPE type, size_t size, Activation activation){
    ...
    layer->input = matrix::createMatrix(1, size, row);
    return layer;
}
...
static Connection* createConnection(Layer* from, Layer* to){
    ...
    connection->weights = matrix::createMatrix(from->size, to->size, weights_data);
    float* bias_data = (float*)malloc(sizeof(float) * to->size);
    connection->bias = matrix::createMatrix(1, to->size, bias_data);
    return connection;
}

// weights are guassian random using fan-in method
// biases are 0
static void initializeConnection(Connection* connection, std::minstd_rand &gen){
    int i, j;
    for (i = 0; i < connection->bias->cols; i++){
        matrix::setMatrix(connection->bias, 0, i, 0);
    }
    for (i = 0; i < connection->weights->rows; i++){
        for (j = 0; j < connection->weights->cols; j++){
            int neuronsIn = connection->weights->rows;
            matrix::setMatrix(connection->weights, i, j, (box_muller(gen)) / sqrt(neuronsIn));
        }
    }
}
...
void destroyLayer(Layer* layer) {
    if (layer->input != NULL) matrix::destroyMatrix(layer->input);
    free(layer);
}

void destroyConnection(Connection* connection){
    matrix::destroyMatrix(connection->weights);
    matrix::destroyMatrix(connection->bias);
    free(connection);
}
