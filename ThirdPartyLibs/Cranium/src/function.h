...
void sigmoid(Matrix* input){
    ...
    for (i = 0; i < input->rows; i++){
        for (j = 0; j < input->cols; j++){
            matrix::setMatrix(input, i, j, sigmoidFunc(matrix::getMatrix(input, i, j)));
        }
    }
}
...
void relu(Matrix* input){
    ...
    for (i = 0; i < input->rows; i++){
        for (j = 0; j < input->cols; j++){
            matrix::setMatrix(input, i, j, reluFunc(matrix::getMatrix(input, i, j)));
        }
    }
}
...
void tanH(Matrix* input){
    ...
    for (i = 0; i < input->rows; i++){
        for (j = 0; j < input->cols; j++){
            matrix::setMatrix(input, i, j, tanHFunc(matrix::getMatrix(input, i, j)));
        }
    }
}
...
void softmax(Matrix* input){
    ...
    for (i = 0; i < input->rows; i++){
        ...
        int j;
        for (j = 0; j < input->cols; j++){
            summed += expf(matrix::getMatrix(input, i, j));
        }
        for (j = 0; j < input->cols; j++){
            matrix::setMatrix(input, i, j, expf(matrix::getMatrix(input, i, j)) / summed);
        }
    }
}
...
static float box_muller(std::minstd_rand &gen) {
