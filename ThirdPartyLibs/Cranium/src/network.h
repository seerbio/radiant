...
static Network* createNetwork(size_t numFeatures, size_t numHiddenLayers, size_t* hiddenSizes,
	Activation* hiddenActivations, size_t numOutputs, Activation outputActivation, std::minstd_rand &gen){
...
void forwardPass(Network* network, Matrix* input){
    ...
    matrix::destroyMatrix(network->layers[0]->input);
	network->layers[0]->input = matrix::copy(input);
    int i;
    Matrix* tmp,* tmp2;
    for (i = 0; i < network->numConnections; i++){
        tmp = matrix::multiply(network->layers[i]->input, network->connections[i]->weights);
        tmp2 = matrix::addToEachRow(tmp, network->connections[i]->bias);
        matrix::destroyMatrix(network->connections[i]->to->input);
        network->connections[i]->to->input = tmp2;
        matrix::destroyMatrix(tmp);
...
static void forwardPassDataSet(Network* network, DataSet* input){
    Matrix* dataMatrix = matrix::dataSetToMatrix(input);
    forwardPass(network, dataMatrix);
    matrix::destroyMatrix(dataMatrix);
}
...
static float crossEntropyLoss(Network* network, Matrix* prediction, DataSet* actual, float regularizationStrength){
    ...
    for (i = 0; i < prediction->rows; i++){
        ...
        for (j = 0; j < prediction->cols; j++){
            cur_err += actual->data[i][j] * logf(MAX(FLT_MIN, matrix::getMatrix(prediction, i, j)));
        }
        total_err += cur_err;
    }
    float reg_err = 0;
        ...
        for (i = 0; i < network->numConnections; i++){
            ...
            for (j = 0; j < weights->rows; j++){
                for (k = 0; k < weights->cols; k++){
                    reg_err += matrix::getMatrix(weights, j, k) * matrix::getMatrix(weights, j, k);
                }
            }
        }
    }
...
static float crossEntropyLoss(Network* network, DataSet* data, DataSet* classes) {
	assert(data->rows == classes->rows);
	assert(classes->cols == network->layers[network->numLayers - 1]->size);
	forwardPassDataSet(network, data);
	Matrix* output = getOuput(network);
	float result = crossEntropyLoss(network, output, classes, 0.0);
	return result;
}
...
float meanSquaredError(Network* network, Matrix* prediction, DataSet* actual, float regularizationStrength){
    ...
    for (i = 0; i < prediction->rows; i++){
        float cur_err = 0;
        for (j = 0; j < prediction->cols; j++){
            float tmp = actual->data[i][j] - matrix::getMatrix(prediction, i, j);
            cur_err += tmp * tmp;
        }
        total_err += cur_err;
    }
...
int* predict(Network* network){
    ...
    for (i = 0; i < outputLayer->input->rows; i++){
        ...
        for (j = 1; j < outputLayer->size; j++){
            if (matrix::getMatrix(outputLayer->input, i, j) > matrix::getMatrix(outputLayer->input, i, max)){
                max = j;
            }
        }
        predictions[i] = max;
    ...
    for (k = 0; k < network->numConnections; k++){
        ...
        for (i = 0; i < con->weights->rows; i++){
            for (j = 0; j < con->weights->cols; j++){
                fprintf(fp, "%a\n", matrix::getMatrix(con->weights, i, j));
            }
        }
    }
...
Network* readNetwork(char* path) {
	...
	// get layer sizes
	size_t * layerSizes = (size_t*)alloca(numLayers * sizeof(size_t));
	...
	Activation * funcs = (Activation*)alloca((numLayers - 1) * sizeof(Activation));
	char funcString[50];
	...
	if (numHiddenLayers > 0) {
		size_t * hiddenSizes = (size_t*)alloca((numLayers - 2) * sizeof(size_t));
		for (i = 0; i < numLayers - 2; i++) {
			hiddenSizes[i] = layerSizes[1 + i];
		}
		Activation * hiddenFuncs = (Activation*)alloca((numLayers - 2) * sizeof(Activation));
		for (i = 0; i < numLayers - 2; i++) {
			hiddenFuncs[i] = funcs[i];
		}
		network = createNetwork(inputSize, numHiddenLayers, hiddenSizes, hiddenFuncs, outputSize, outputFunc, gen);
	}
	else {
		network = createNetwork(inputSize, 0, NULL, NULL, outputSize, outputFunc, gen);
	}
